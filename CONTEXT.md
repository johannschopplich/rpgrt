...
Reads and writes the binary LCF files of RPG Maker 2000/2003 games (maps, database, map tree, saves) in pure TypeScript, and ships a CLI for JSON conversion and translation workflows. Reimplements the format knowledge of EasyRPG's C++ liblcf without wrapping it.
...
**Base struct**:
A struct whose fields another struct inherits – only `SaveMapEventBase`, flattened into `SavePartyLocation`, `SaveVehicleLocation`, and `SaveMapEvent` at generation, its chunks kept ahead of the derived struct's own in liblcf's emission order.
...
**Corpus**:
Real, third-party game files used locally for byte-fidelity testing. Lives in `test/corpus/`, never committed; CI fetches the EasyRPG TestGame repository into it at a pinned commit.

**Fixture**:
A committed, self-authored test file in `test/fixtures/`, generated by the builders in `test/fixtures/build.ts` (`pnpm run make-fixtures`). Never a copyrighted game file.
...
**Control code**:
An inline command inside message text, e.g. `\c[3]`, `\n[1]`. Zero display width; every code must survive translation byte-exact – the multiset is enforced, the order follows the translation.
